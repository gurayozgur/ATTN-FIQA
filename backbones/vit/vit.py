...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_raw = (q @ k.transpose(-2, -1)) * self.scale  # Pre-softmax scores
        attn = attn_raw.softmax(dim=-1)
        attn = self.attn_drop(attn)
        x = (attn @ v).transpose(1, 2).reshape(batch_size, num_token, embed_dim)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, attn_raw  # Return pre-softmax attention for quality assessment
...
class Block(nn.Module):
    ...
    def forward(self, x):
        attn_weights = self.attn(self.norm1(x))
        if isinstance(attn_weights, tuple):
            x_attn, attn = attn_weights
            x = x + self.drop_path(x_attn)
        else:
            x = x + self.drop_path(attn_weights)
            attn = None
...
class VisionTransformer(nn.Module):
    ...
    @torch.no_grad()
    def calculate_attnfiqa(self, x: torch.Tensor):
        """
        Calculate ATTN-FIQA for input tensor x.

        Args:
            x: Input tensor of shape (batch_size, channels, height, width)
        Returns:
            Quality scores tensor of shape (batch_size,)
        """
        ...
        # pre-softmax attn is of shape [batch_size, num_heads, num_tokens, num_tokens]
        # concatenate to shape [batch_size, num_heads * num_tokens * num_tokens]
        all_attention_values = attn.reshape(attn.shape[0], -1)
        # Average pre-softmax attention values across heads and tokens
        scores = all_attention_values.mean(dim=1)
